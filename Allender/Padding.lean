...
The manuscript encodes the `t`-th block-output circuit at input length `n` as a
circuit of input length `N(n,t)=n^(d+2)+t`.  This file verifies that the ranges
for different positive `n` are disjoint whenever `t<n^d`.
-/

namespace Allender

/-- Padded input length used to combine all block circuits into one family. -/
def paddedLength (d n t : Nat) : Nat := n ^ (d + 2) + t
...
theorem paddedLength_injective_on_ranges {d n m t s : Nat}
  ...
  have hnm : n = m := by
    ...
    · have hsucc : n + 1 ≤ m := by omega
      have hbaseMono : (n + 1) ^ (d + 2) ≤ m ^ (d + 2) :=
        Nat.pow_le_pow_left hsucc _
      have hleft : paddedLength d n t < (n + 1) ^ (d + 2) := by
        unfold paddedLength
        exact (Nat.add_lt_add_left ht _).trans (padding_gap d n hn)
      have hright : (n + 1) ^ (d + 2) ≤ paddedLength d m s := by
        unfold paddedLength
    ...
    · have hsucc : m + 1 ≤ n := by omega
      have hbaseMono : (m + 1) ^ (d + 2) ≤ n ^ (d + 2) :=
        Nat.pow_le_pow_left hsucc _
      have hleft : paddedLength d m s < (m + 1) ^ (d + 2) := by
        unfold paddedLength
        exact (Nat.add_lt_add_left hs _).trans (padding_gap d m hm)
      have hright : (m + 1) ^ (d + 2) ≤ paddedLength d n t := by
        unfold paddedLength
        exact hbaseMono.trans (Nat.le_add_right _ _)
      rw [← heq] at hleft
      exact (Nat.not_lt_of_ge hright hleft).elim
